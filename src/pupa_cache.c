...
#if (_PUPA_DARWIN)
static int _pupa_cache_item_compare(void *p1, const void *p2, const void *arg);
#else
static int _pupa_cache_item_compare(const void *p1, const void *p2, void *arg);
#endif
...
static int  pupa_cache_value_compaction(pupa_ctx_t *ctx, pupa_str_t *value,
                                        char **address);
static int  pupa_cache_key_compaction(pupa_ctx_t *ctx, pupa_str_t *key,
                                      char **address);
...
int pupa_cache_get(pupa_ctx_t *ctx, pupa_str_t *key, pupa_str_t *value)
{
    ...
    p_cache_item = bsearch(&cache_item_wrapper.cache_item, ctx->cache_items,
                           p_item_section->used, sizeof(pupa_cache_item_t),
...
int pupa_cache_set(pupa_ctx_t *ctx, pupa_str_t *key, pupa_str_t *value)
{
    ...
    p_cache_item = bsearch(&cache_item_wrapper.cache_item,
                           ctx->cache_items_mirror, p_item_section->used,
                           sizeof(pupa_cache_item_t), pupa_cache_item_compare);
    if (p_cache_item == NULL) {
        DEBUG_LOG("Add key : %.*s - %.*s", key->len, key->data, value->len,
                  value->data);
        ret = pupa_cache_item_add(
            ctx, ctx->cache_items_mirror + p_item_section->used, key, value);
    } else {
    ...
    if (p_cache_item == NULL) {
        ...
        qsort_r(ctx->cache_items_mirror, ctx->cache_hdr->item_section.used,
                ...
                _pupa_cache_item_compare);
#else
        qsort_r(ctx->cache_items_mirror, ctx->cache_hdr->item_section.used,
                sizeof(pupa_cache_item_t), _pupa_cache_item_compare,
                &cache_item_wrapper);
#endif
...
int pupa_cache_stats(pupa_ctx_t *ctx, pupa_str_t *stat)
{
    ...
    stat->len = sprintf(
        ...
        "\t}\n"
        "}\n",
        ctx->cache_hdr->item_section.size, ctx->cache_hdr->item_section.used,
        ctx->cache_hdr->item_section.id,
        ctx->cache_hdr->item_section.sec1_offset,
        ctx->cache_hdr->item_section.sec2_offset,
        ctx->cache_hdr->key_section.size, ctx->cache_hdr->key_section.used,
        ctx->cache_hdr->key_section.id, ctx->cache_hdr->key_section.sec1_offset,
        ctx->cache_hdr->key_section.sec2_offset,
        ctx->cache_hdr->value_section.size, ctx->cache_hdr->value_section.used,
        ctx->cache_hdr->value_section.id,
